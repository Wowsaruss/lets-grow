...
const ai = {
    async generatePlantData(commonName: string, variety: string) {
        ...
        const prompt = `You are a gardening expert. Given the plant name and variety, generate a JSON object with all the following fields for a plant database: scientific_name, description, plant_family_id (number), plant_type_id (number), days_to_germination (number), days_to_harvest (number), indeterminate (boolean), germination_temp_high (number), germination_temp_low (number), light (string), water (string), soil (string), perennial (boolean), heirloom (boolean), hybrid (boolean), open_pollinated (boolean), self_pollinated (boolean), spacing (number), row_spacing (number), pruning (string), sowing_depth (number). Use realistic values. Example input: Tomato, Brandywine. Output only the JSON object.`;
